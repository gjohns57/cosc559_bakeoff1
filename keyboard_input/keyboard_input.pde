...
HashSet<Character> keys_down = new HashSet<Character>();
...
int numRepeats = 5; //sets the number of times each button repeats in the test
...
void keyReleased()
{
  ...
  if (keys_down.isEmpty()) {
     ...
     if (mode == 0) {
        ...
        Character[] row_keys = {'3', 'e', 'd', 'c'};
        Character[] col_keys = {'v', 'b', 'n', 'm'};
  
        System.out.println("Key released");
        for (Character c : keys_pressed) {
           System.out.print(c); 
        }
        System.out.println("");
  
        for (int i = 0; i < 4; i++) {
          if (keys_pressed.contains(row_keys[i])) {
            rows.add(i);
          }
          if (keys_pressed.contains(col_keys[i])) {
            cols.add(i);
          }
        }
